...
docker images
```
Busca `mi-react-app` y `node`. Notarás que `mi-react-app` (basada en `nginx:stable-alpine`) es significativamente más pequeña que la imagen `node:22-alpine`. ¡Hemos logrado nuestro objetivo!
